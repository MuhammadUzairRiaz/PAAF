...
at negligible cost. Each rotatable group is scanned over 144 uniform angles and
...
\item \textbf{Minimisation is optional; equilibration is not done.}
...
energy}~\cite{biovia}. PAAF builds the guess structure and can, when
\emph{Relax} is ticked, run a relaxation with the real force field: a soft
push-off followed by conjugate-gradient minimisation in LAMMPS, or steepest
descent in GROMACS (\texttt{paaf/cell/relax.py}). The geometric push-off of
Section~\ref{sec:backmap} is not itself a minimisation and involves no force
field. PAAF does not run NPT equilibration: \emph{users must compress and
equilibrate before quoting any property from a PAAF cell.}
